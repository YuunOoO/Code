...
void preorder(drzewo* node) //wypisywanie preorder
{
	...
	{
		cout << node->var << endl; //wypisz wartosc
		inorder(node->left);	//przejdz do lewego poddrzewa
		inorder(node->right);	//przejdz do prawego poddrzewa
	}
}
void postorder(drzewo* node) //wypisywanie postorder
{
	...
	{
		inorder(node->left);	//przechodzimy do lewego poddrzewa
		inorder(node->right);	//przechodzimy do prawego poddrzewa
		cout << node->var << endl;	//wypisywanie
	}
}
...
int main()
{
	int tab[N] = { 38, 96, 40, 29, 61, 21, 23, 31, 15, 98 }; //nasza tablica
